...
def main():
    ...
    if args.task and args.task == 'ct_recon':
        ref_acts = evaluator.read_activations_npy(args.ref_batch)
    else:
...
class Evaluator:
    ...
    def read_activations(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        with open_npz_array(npz_path, "generated") as reader:
            return self.compute_activations(reader.read_batches(self.batch_size))

    def read_activations_npy(self, npy_path: str) -> Tuple[np.ndarray, np.ndarray]:
        with open_npz_array(npy_path, None) as reader:
            return self.compute_activations(reader.read_batches(self.batch_size))
    ...
    def read_statistics(
        ...
        if obj is dict and "mu" in list(obj.keys()):
...
def open_npz_array(path: str, arr_name: str) -> NpzArrayReader:
    if arr_name is None:
        arr = np.load(path, 'r')[:, :, :, None]
        yield MemoryNpzArrayReader(arr)
        return
